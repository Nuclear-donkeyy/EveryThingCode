...
C / C++ 的第一版学习基线是 **C23 / C++23**，工具链入口是 **C23/C++23 compiler**。它主要用于：系统、游戏、音视频、数据库、性能核心。
...
- 类型模型：静态类型、手动内存、模板元编程。
- 并发模型：线程、原子、协程、执行器生态。
- 内存与资源：结合语言自己的生命周期管理方式学习，不把所有语言都套成同一种范式。

## 标准库与包管理

包管理入口：CMake / Conan / vcpkg。第一版案例优先使用标准库，只有框架章节才引入生态依赖。

## 错误处理

C 错误码，C++ 异常/expected/RAII。学习时关注错误如何被表达、传播、恢复，以及如何避免把异常路径藏在业务逻辑里。
