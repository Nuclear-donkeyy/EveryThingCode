...
Ruby 的第一版学习基线是 **4.0.x**，工具链入口是 **Ruby stable**。它主要用于：Web 产品、自动化、DSL、脚本工具。
...
- 类型模型：动态类型、duck typing、RBS/Sorbet 可选。
- 并发模型：Fiber、Ractor、线程。
- 内存与资源：结合语言自己的生命周期管理方式学习，不把所有语言都套成同一种范式。

## 标准库与包管理

包管理入口：Bundler / RubyGems。第一版案例优先使用标准库，只有框架章节才引入生态依赖。

## 错误处理

异常、ensure、显式领域错误。学习时关注错误如何被表达、传播、恢复，以及如何避免把异常路径藏在业务逻辑里。
