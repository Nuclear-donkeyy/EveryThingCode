...
Dart 的第一版学习基线是 **3.12.x**，工具链入口是 **Dart 3.12**。它主要用于：Flutter 应用、跨平台 UI、客户端工具。
...
- 类型模型：静态类型、sound null safety、泛型。
- 并发模型：Future、Stream、isolate。
- 内存与资源：结合语言自己的生命周期管理方式学习，不把所有语言都套成同一种范式。

## 标准库与包管理

包管理入口：pub。第一版案例优先使用标准库，只有框架章节才引入生态依赖。

## 错误处理

Exception/Error、try/catch、sealed 结果类型。学习时关注错误如何被表达、传播、恢复，以及如何避免把异常路径藏在业务逻辑里。
