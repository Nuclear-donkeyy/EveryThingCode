...
Swift 的第一版学习基线是 **6.3.x**，工具链入口是 **Swift 6.3**。它主要用于：Apple 平台、服务端、CLI、系统工具。
...
- 类型模型：静态类型、协议、值语义、泛型。
- 并发模型：async/await、actor、Task。
- 内存与资源：结合语言自己的生命周期管理方式学习，不把所有语言都套成同一种范式。

## 标准库与包管理

包管理入口：Swift Package Manager。第一版案例优先使用标准库，只有框架章节才引入生态依赖。

## 错误处理

throws、Result、defer。学习时关注错误如何被表达、传播、恢复，以及如何避免把异常路径藏在业务逻辑里。
