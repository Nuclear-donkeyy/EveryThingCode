...
Rust 的第一版学习基线是 **1.96.x**，工具链入口是 **Rust stable**。它主要用于：系统编程、性能服务、嵌入式、WebAssembly。
...
- 类型模型：静态类型、代数数据类型、trait。
- 并发模型：所有权、Send/Sync、async runtime。
- 内存与资源：结合语言自己的生命周期管理方式学习，不把所有语言都套成同一种范式。

## 标准库与包管理

包管理入口：Cargo。第一版案例优先使用标准库，只有框架章节才引入生态依赖。

## 错误处理

Result、Option、?、panic 边界。学习时关注错误如何被表达、传播、恢复，以及如何避免把异常路径藏在业务逻辑里。
