...
Go 的第一版学习基线是 **1.26.3**，工具链入口是 **Go 1.26.3**。它主要用于：云原生、微服务、CLI、基础设施。
...
包管理入口：Go modules。第一版案例优先使用标准库，只有框架章节才引入生态依赖。

## 错误处理

显式 error 返回、errors.Is/As、defer。学习时关注错误如何被表达、传播、恢复，以及如何避免把异常路径藏在业务逻辑里。
