...
def main() -> int:
    ...
    failures: list[str] = []
    readmes = sorted((ROOT / "languages").glob("*/syntax/examples/syntax-tour/README.md"))
    if not readmes:
        failures.append("no syntax-tour README files found")
    ...
    for readme in readmes:
        ...
        if shutil.which(tool) is None:
            print(f"SKIP: {tool} is not installed for {rel}")
            continue
    ...
    print("OK: syntax example command scan completed")
    return 0
